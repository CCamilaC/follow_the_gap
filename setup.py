...
Updated version:
 - Two-phase behavior: ROTATE then MOVE.
 - English debug messages with detailed context.
"""
...
class GapFollowerNode(Node):
    def __init__(self):
        ...
        # motion state
        self.state = "IDLE"  # can be ROTATING, MOVING, or IDLE
        self.moving = False
        ...
        # timer for publishing cmd_vel (10 Hz)
        self.motion_timer = self.create_timer(0.1, self.motion_timer_callback)

        self.get_logger().info('✅ GapFollowerNode started — waiting for /scan_180 messages')
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        if result['status'] == 'SAFE':
            ...
            if gap_distance is None or gap_distance <= 0.0:
                self.get_logger().warn("⚠️ Gap detected but invalid distance value — ignoring.")
                return

            # compute travel time and rotation time
            travel_time = min(gap_distance / self.linear_speed, 10.0)
            self.travel_time = travel_time

            target_angle_rad = math.radians(steering_angle)
            rotate_time = min(abs(target_angle_rad) / self.rotate_speed, 3.0)
            ...
            self.get_logger().info(
                f"[SAFE GAP FOUND] Steering angle: {steering_angle:.1f}°, "
                f"Gap width: {result['gap_width']:.2f} m, "
                f"Avg. distance: {gap_distance:.2f} m, "
                f"Safety margin: {result['safety_margin']:.2f} m"
            )
            self.get_logger().info(
                f"🌀 Starting ROTATION phase: turning {steering_angle:.1f}° "
                f"for {rotate_time:.2f} seconds at {self.rotate_speed:.2f} rad/s"
            )

            self.publish_marker_throttled(steering_angle, gap_distance)

        else:
            self.get_logger().warn("🚫 No traversable gap found — robot will remain stopped.")
            self.stop_robot()
    ...
    def motion_timer_callback(self):
        ...
        if self.state == "ROTATING":
            if now_sec < self.rotate_end_time_sec:
                ...
                self.get_logger().info(
                    f"✅ ROTATION completed — transitioning to MOVING phase. "
                    f"Moving straight for {self.travel_time:.2f}s at {self.linear_speed:.2f} m/s."
                )
    ...
    def stop_robot(self):
        ...
        if self.moving:
            self.get_logger().info("🛑 STOP command issued — robot motion halted.")
    ...
    def publish_marker(self, angle_deg, distance):
        ...
        self.get_logger().info(
            f"📍 Visualization marker published in {marker.header.frame_id}: "
            f"angle={angle_deg:.1f}°, distance={distance:.2f} m"
        )
