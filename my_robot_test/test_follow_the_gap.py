...
Two-phase movement version:
1) Rotates while stationary until the detected gap angle is reached.
2) Moves in a straight line to the center of the gap.
"""

import math
import rclpy #ROS2 Python
from rclpy.node import Node
from sensor_msgs.msg import LaserScan #LiDAR reading structure (minimum angle, maximum angle, increments, ranges)
from visualization_msgs.msg import Marker #for viewing in RViz (arrow)
...
def find_disparities(ranges, threshold):
    ...
    for i in range(len(ranges) - 1):
        r1 = ranges[i] if not math.isinf(ranges[i]) else 100.0 #Here we replace inf with 100.0 m so as not to interfere with the calculation of the difference.
